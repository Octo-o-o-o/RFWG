...
- ✅ **语音 SILK→WAV 解码**：已用 `pilk` 合成 SILK 往返自测（造带腾讯头的 silk 再解回 WAV，头字节 `02 23 21 53 49 4c 4b 5f 56 33` 与真机一致）。
- ⚠️ **语音端到端转写未在真机 + 模型环境跑过**：`transcribe_voice.py` 的 ASR 端用 faster-whisper 稳定接口，但首次需联网下模型。首用建议先 `--dry-run` 看命中、再 `--limit 3` 抽验转写质量。

## 8. 语音转写（可选，本地离线 STT）

仅当要把微信语音转成文字时才需要；**不装不影响其它任何功能**。

```bash
pip install -r requirements-voice.txt      # pilk（SILK 解码）+ faster-whisper（本地 ASR）
```

- **数据来源/格式**：见 `wechat-local-data.md §6`（明文 SILK v3，无需密钥）。
- **模型**：faster-whisper 首次运行联网下载模型（`small` 约 480MB，中文建议 small 以上）；下好后加 `--offline` 完全断网，或 `--model-dir` 指向已下目录。国内慢可设 `HF_ENDPOINT=https://hf-mirror.com`。
- **完全本地**：不调用任何云 ASR，音频与转写文本不外发；中间 WAV 转写后即删。可接受少量错字（下游调研影响小）。
- **Mac 提速（可选）**：Apple Silicon 另装 `mlx-whisper`；或 `whisper.cpp`（Metal）。**中文更准（可选）**：`FunASR`（依赖较重）。
