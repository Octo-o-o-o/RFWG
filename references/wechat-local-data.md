...
## 6. 语音消息（SILK v3，明文，可本地转写）

微信语音是 **SILK v3** 编码，**明文、无加密**（与图片 V2、数据库 SQLCipher 无关，无需任何密钥）。已在真机确认：

- **位置**：`cache/<YYYY-MM>/Message/<md5(room)>/VoiceTemp/<md5(room)>_<unixts>_N.silk`——与缩略图 `Thumb/` 同级同构。
- **文件头**：`02 23 21 53 49 4c 4b 5f 56 33 0a` = `\x02#!SILK_V3\n`（微信在标准 `#!SILK_V3` 前多一个 `\x02`，解码前剥掉）。
- **命名**：前缀是 room 的 md5（= `md5(username)`，与父目录一致），中段是**秒级 Unix 时间戳**（可按群 + 时间定位），故无需查数据库即可对上发送群与时间。
- **覆盖范围（重要）**：`VoiceTemp` 大概率只含**被播放过**的语音（类比 `Thumb` 只覆盖被加载过的图），**非全量**；未播放过的语音可能不在此、或以加密形式存于 `msg/attach`（本项目暂未覆盖）。
- **转写链路**：`scripts/transcribe_voice.py` 收集 → `pilk.decode_to_wav`（SILK→WAV，剥 `\x02`）→ `faster-whisper` 本地转写（CPU 可跑、可离线）。SILK→WAV 解码已用 `pilk` 合成往返验证（造带腾讯头的 silk 再解回 WAV，头字节与真机一致）。安装与离线见 `toolchain-setup.md §8`。
