...
5. 把群里的**图片**取下来（用微信缓存里的已解密缩略图）、拼成索引图**逐张判读**，有价值的留下、没价值的回档；
6. 需要时解密 **朋友圈（sns.db）** 作为补充材料；
...
- [`wechat-cli`](https://github.com/freestylefly/wechat-cli)（`@canghe_ai/wechat-cli`）：微信本地数据查询/解密。
  ```bash
  npm i -g @canghe_ai/wechat-cli
  wechat-cli init          # macOS 首次需一次性授权（wxkey 提取数据库密钥）
  ```
- Python 3 + `pycryptodome`（解密）+ `pillow`（图片拼图）：
  ```bash
  pip3 install --break-system-packages pycryptodome pillow
  ```
- 读图与浏览器验收：AI 侧的读图能力 + Playwright（或本机 Chrome 无头）。
- 目前脚本按 **macOS 微信 4.x** 的本地布局编写；其它平台需按 `references/wechat-local-data.md` 适配路径。
...
├── LICENSE
├── scripts/                        # 可复用脚本（确定性步骤）
│   ├── wxcommon.py                 # 共享库：定位本地库 / 消息清洗 / 密钥加载
...
│   └── decrypt_images_v2.py        # （进阶）已知图片密钥时解 V2 原图
├── references/                     # 按需查阅的技术底料
│   ├── wechat-local-data.md        # 微信本地结构 / SQLCipher / V2 图片加密
